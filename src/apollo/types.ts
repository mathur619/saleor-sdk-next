...
export type CreateJusPayOrder = {
  ...
  errors: Array<Error>;
  /** A Juspay order object. */
  juspayOrder: Maybe<JuspayOrderType>;
  juspayErrors: Array<JuspayError>;
};
...
export type JuspayCreateOrderInput = {
  /** Checkout ID. */
  checkoutId: Scalars['ID'];
};
...
export type Mutation = {
  ...
  /** Creates an order on Juspay. */
  juspayOrderCreate: Maybe<CreateJusPayOrder>;
  /** Check Order status on Juspay. */
...
export type MutationJuspayOrderCreateArgs = {
  input: JuspayCreateOrderInput;
};
...
export type CreateJuspayOrderMutationVariables = Exact<{
  input: JuspayCreateOrderInput;
}>;


export type CreateJuspayOrderMutation = { juspayOrderCreate: Maybe<{ errors: Array<Pick<Error, 'field' | 'message'>>, juspayOrder: Maybe<Pick<JuspayOrderType, 'id' | 'orderId' | 'amount' | 'status' | 'paymentLink'>>, juspayErrors: Array<Pick<JuspayError, 'field' | 'message' | 'code'>> }> };
...
export const CreateJuspayOrderDocument = gql`
    mutation CreateJuspayOrder($input: JuspayCreateOrderInput!) {
  juspayOrderCreate(input: $input) {
    errors {
      field
      message
    }
    ...
    `;
export type CreateJuspayOrderMutationFn = Apollo.MutationFunction<CreateJuspayOrderMutation, CreateJuspayOrderMutationVariables>;

/**
 * __useCreateJuspayOrderMutation__
 *
 * To run a mutation, you first call `useCreateJuspayOrderMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useCreateJuspayOrderMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [createJuspayOrderMutation, { data, loading, error }] = useCreateJuspayOrderMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useCreateJuspayOrderMutation(baseOptions?: Apollo.MutationHookOptions<CreateJuspayOrderMutation, CreateJuspayOrderMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<CreateJuspayOrderMutation, CreateJuspayOrderMutationVariables>(CreateJuspayOrderDocument, options);
      }
export type CreateJuspayOrderMutationHookResult = ReturnType<typeof useCreateJuspayOrderMutation>;
export type CreateJuspayOrderMutationResult = Apollo.MutationResult<CreateJuspayOrderMutation>;
export type CreateJuspayOrderMutationOptions = Apollo.BaseMutationOptions<CreateJuspayOrderMutation, CreateJuspayOrderMutationVariables>;
export const CheckJuspayOrderStatusDocument = gql`
    mutation CheckJuspayOrderStatus($input: JuspayCreateOrderInput!) {
